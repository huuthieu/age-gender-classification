...
class AgeGenderHelper:
	...
	def visualizeAge(agePreds, le):
		...
		for (i, j) in enumerate(idxs):
			...
			ageLabel = le.inverse_transform(j.reshape(-1, 1)) # Python 2.7
			# ageLabel = le.inverse_transform(j.reshape(-1, 1)).decode("utf-8")
	...
	def visualizeGender(genderPreds, le):
		...
		for (i, j) in enumerate(idxs):
			gender = le.inverse_transform(j.reshape(-1,1))
			# construct the text for the prediction
